...
class MobiFallDataLoader:
    ...
    def _prepare_dataset_from_dir(self, folder=''):
        ...
        files = [file for file in files if "acc" in file]
        print(files)
        num_trails = len(files)
        num_trails_per_sensor = num_trails // self.sensor_num
    ...
    def _interpolate_sensor_data(self, features):
        '''
    ...
    def _get_files(self, dir_name='', sort=False):
        ...
        if sort:
            assert len(files) // self.sensor_num < 10, "Add functionality for >=10 trails"
            return sorted(files)
        return files

    def _get_trails(self, folder=''):
        files = self._get_files(folder, sort=True)
        assert len(files) % self.sensor_num == 0, "Inconsistent number of sensors in " + folder
